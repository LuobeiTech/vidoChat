...
- (void)didSendMessageWithVideoMinConverPhoto:(UIImage *)videoConverPhoto videoPath:(NSString *)videoPath  {
    DLog(@"send Min videoPath : %@  videoConverPhoto : %@", videoPath, videoConverPhoto);
    if ([self.delegate respondsToSelector:@selector(didSendVideoMinConverPhoto:videoPath:fromSender:onDate:)]) {
        [self.delegate didSendVideoMinConverPhoto:videoConverPhoto videoPath:videoPath fromSender:self.messageSender onDate:[NSDate date]];
    }
}
...
- (void)finishRecorded {
    ...
    [self.voiceRecordHelper stopRecordingWithStopRecorderCompletion:^{
        ...
        NSString *mp3Path = [WHHSoudMp3Util toMp3Path:weakSelf.voiceRecordHelper.recordPath];
        [weakSelf didSendMessageWithVoice:mp3Path voiceDuration:weakSelf.voiceRecordHelper.recordDuration];
    }];
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(savePathNotification:) name:@"sendAVpathUrlNotification" object:nil];
    
}
-(void)savePathNotification:(NSNotification*)notification{
    NSDictionary *nameDictionary = [notification userInfo];
    
    WEAKSELF
    UIImage *image = (UIImage*)[nameDictionary objectForKey:@"videoImage"];
    NSString *videoPath = (NSString*)[nameDictionary objectForKey:@"videoPath"];
    [weakSelf didSendMessageWithVideoConverPhoto:image videoPath:videoPath];
//    [self performSelectorOnMainThread:@selector(sendVideoMessage:) withObject:nameDictionary waitUntilDone:YES];
}
//-(void)sendVideoMessage:(NSDictionary*)nameDictionary{
//    
//}
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"sendAVpathUrlNotification" object:nil];
}
@end
