...
- (void)viewDidLoad
{
    ...
    self.submitButton = [[UIButton alloc] initWithFrame: CGRectMake(DEVICE_SIZE.width-44, 0, 44, 44)];

    [_submitButton setTitle:@"确定" forState:UIControlStateNormal];
    [_submitButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_submitButton addTarget:self action:@selector(sumbitButton:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)sumbitButton:(UIButton *)button
{
//    [self dismissViewControllerAnimated:YES completion:(void(^)((UIImage *)image ,(NSString*) patUrl)finishBlock{
//    
//        patUrl =_videoFileURL;
//    }]);
    UIImage *thumbnailImage = [XHMessageVideoConverPhotoFactory videoConverPhotoWithVideoPath:[self.videoFileURL path]];
    
    [self dismissViewControllerAnimated:YES completion:^{
   
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sendAVpathUrlNotification" object:self userInfo:@{@"videoImage":thumbnailImage,@"videoPath": [self.videoFileURL path] }];
